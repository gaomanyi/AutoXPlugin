...
class AutoXToolWindowPanel(private val project: Project) : JPanel(), Disposable, ConnectionListener {
    ...
    private var statusMessage: String = I18n.msg("service.hint.none")
    ...
    // 修改启动和结束服务按钮
    private val startServerButton = JButton(I18n.msg("service.start"), AllIcons.Actions.Execute).apply {
        addActionListener { startServer() }
        alignmentX = Component.LEFT_ALIGNMENT
        border = JBUI.Borders.empty(5, 10)
    }

    private val stopServerButton =
        JButton(I18n.msg("service.stop"), IconLoader.getDisabledIcon(AllIcons.Actions.Suspend)).apply {
    ...
    // 修改停止所有脚本按钮
    private val stopAllScriptsButton = JButton(I18n.msg("script.all.stop"), stopDisabledIcon).apply {
        isEnabled = false
        addActionListener {
            sendStopAllCommand()
        }
    ...
    private val deviceTable = JBTable(deviceTableModel).apply {
        ...
        // 设置表格为空时的显示信息
        emptyText.text = I18n.msg("device.connect.none")
    }
    ...
    private val qrCodePanel = object : JPanel() {
        override fun paintComponent(g: Graphics) {
            ...
            if (image != null) {
                ...
                g2d.drawString(I18n.msg("service.please.start"), 10, height / 2)
            }
        }
    ...
    // 清除日志按钮
    private val clearLogButton = JButton(I18n.msg("device.log.clear")).apply {
        addActionListener {
            clearLog()
        }
    }
    ...
    init {
        panel = panel {
            row {
                label("AutoX WebSocket " + I18n.msg("service.name"))
                    .bold()
                    .align(Align.CENTER)
            }
            ...
            // 添加设备列表区域
            group(I18n.msg("device.connect.list")) {
                row {
    ...
    private fun syncServiceState() {
        ...
        if (webSocketService.isRunning()) {
            ...
            if (serverAddress != null) {
                ...
                statusMessage = I18n.msg("service.hint") + " $serverAddress"
                ...
                statusMessage = I18n.msg("service.start.warn.hint")
            }

            // 更新设备列表
            updateDevicesList()
        } else {
            statusMessage = I18n.msg("service.hint.none")
            qrCodeImage = null
            serverAddress = null
    ...
    private fun startServer() {
        ...
        if (webSocketService.isRunning()) {
            ...
            statusMessage = I18n.msg("service.start.failure.hint")
        }
    ...
    private inner class DeviceTableModel : AbstractTableModel() {
        private val columnNames = arrayOf(
            I18n.msg("device.name"),
            I18n.msg("device.app.version"),
            I18n.msg("device.action")
        )
    ...
    private inner class ButtonRenderer : TableCellRenderer {
        private val button = JButton(I18n.msg("service.disconnect")).apply {
            isOpaque = true
        ...
        override fun getTableCellRendererComponent(
            ...
            button.text = (value as? String) ?: I18n.msg("service.disconnect")
            return button
        }
    }
    ...
    private fun sendStopAllCommand() {
        if (!webSocketService.isRunning() || webSocketService.getConnectedDeviceCount() == 0) {
            NotificationUtil.showWarningNotification(
                project,
                I18n.msg("action.unable.execute.hint"),
                I18n.msg("action.unable.execute.hint.device.none")
            )
            return
        }
        ...
        scope.launch {
            try {
                ...
                NotificationUtil.showInfoNotification(
                    project,
                    I18n.msg("device.send.command.success"),
                    I18n.msg("device.send.command.stop.all", devices.size)
                )
            } catch (e: Exception) {
                logger.error("Error sending stop all command: ${e.message}", e)
                NotificationUtil.showErrorNotification(
                    project,
                    I18n.msg("device.send.command.failure"),
                    I18n.msg("device.send.command.stop.all.failure", e.message ?: "")
                )
            }
        }
    }
